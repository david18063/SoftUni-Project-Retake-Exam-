...
const CategoryContent = () => {
  ...
  const fetchBooks = async () => {
    try {
      const response = await fetch(`http://localhost:5000/books/category/${encodeURIComponent(category)}`);
      if (!response.ok) throw new Error(`HTTP error! Status: ${response.status}`);
      const data = await response.json();
      setBooks(data);
    } catch (error) {
      console.error("❌ Грешка при зареждане на книги по категория:", error);
    }
  };
  ...
  return (
    <div className="center_content">
      <div className="center_title_bar">Книги от категория: {category}</div>
